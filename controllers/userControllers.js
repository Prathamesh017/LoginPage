...
export const updateUser = async (req, res, next) => {
  ...
  const some = await SignedUser.findOneAndUpdate(
    { email: email },
    { age: age }
  );
  const some2 = await SignedUser.findOneAndUpdate(
    { email: email },
    { DOB: dob }
  );
  const some3 = await SignedUser.findOneAndUpdate(
    { email: email },
    { mobile_no: mobile }
  );
  const some4 = await SignedUser.findOneAndUpdate(
    { email: email },
    { Gender: gender }
  );
